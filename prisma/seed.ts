...
async function main() {
  ...
  console.log(`Cleared employees (${clearMs.toFixed(0)}ms)`);

  console.log(`Inserting in batches of ${BATCH_SIZE}...`);
  const insertStart = performance.now();
  let inserted = 0;

  for (let offset = 0; offset < SEED_COUNT; offset += BATCH_SIZE) {
    const batchSize = Math.min(BATCH_SIZE, SEED_COUNT - offset);
    const batch = Array.from({ length: batchSize }, (_, index) =>
      buildEmployeeRow(offset + index, firstNames, lastNames),
    );
    const result = await prisma.employee.createMany({ data: batch });
    inserted += result.count;
    console.log(`Inserted ${inserted}/${SEED_COUNT} employees`);
  }
